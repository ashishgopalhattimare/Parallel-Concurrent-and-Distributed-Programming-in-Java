...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent svertex;
        while((svertex = nodesLoaded.poll()) != null) {

            // Check whether is marked merged or not
            // whether it is taken by some other thread or not
            if(svertex.isDead || !svertex.lock.tryLock()) {
                continue;
            }

            // LOCK ACQUIRED ON THE SOURCE 'VERTEX'

            // Find the Edge with the minimum weight from source 'vertex'
            final Edge<ParComponent> edge = svertex.getMinEdge();

            // Check whether the edge exist or not
            if(edge == null) {
                solution.setSolution(svertex);
                svertex.lock.unlock();
                break;
            } // Single node exists in the graph after merging

            // Find the destination vertex with (source vertex and edge)
            final ParComponent dvertex = edge.getOther(svertex);

            // Try to acquire lock on the destination 'vertex'
            if (!dvertex.isDead && dvertex.lock.tryLock()) {
                dvertex.isDead = true; // mark it merged

                // Merge it with the source 'vertex'
                svertex.merge(dvertex, edge.weight());

                dvertex.lock.unlock();
            }
            svertex.lock.unlock();
            nodesLoaded.add(svertex);
        }
    }
