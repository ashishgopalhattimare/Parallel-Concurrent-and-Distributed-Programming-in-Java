...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int myranks = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD);
        final int size = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD);
        final int nrows = c.getNRows();

        final int rowChunk = (c.getNRows() + size - 1)/size;
        final int startRow = myranks * rowChunk;
        int endRow = (myranks + 1) * rowChunk;
        if(endRow > nrows) {
            endRow = nrows;
        }

        // For simplixity, broadcast all of matrices a & b to all ranks
        mpi.MPI_Bcast(a.getValues(), 0, a.getNRows() * a.getNCols(), 0,
                mpi.MPI_COMM_WORLD);
        mpi.MPI_Bcast(b.getValues(), 0, b.getNRows() * b.getNCols(), 0,
                mpi.MPI_COMM_WORLD);

        // Compute answer for each row assigned to this rank
        ...
        if(myranks == 0) {
            ...
            mpi.MPI_Send(c.getValues(), startRow * c.getNCols(),
                    (endRow-startRow) * c.getNCols(), 0, myranks,
                    mpi.MPI_COMM_WORLD);
        }
    }
}
