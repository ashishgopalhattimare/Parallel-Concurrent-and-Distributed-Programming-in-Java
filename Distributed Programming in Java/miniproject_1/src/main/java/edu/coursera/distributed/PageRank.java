...
public final class PageRank {
            ...
            final JavaPairRDD<Integer, Double> ranks)
    {
         JavaPairRDD<Integer, Double> result =
                 ...
                 .join(ranks)
                 .flatMapToPair(data -> {
                     Integer websiteId = data._1;
                     Tuple2<Website, Double> value = data._2;
                     Website edges = value._1();
                     Double currentRank = value._2();

                     List<Tuple2<Integer, Double>> contib = new LinkedList<>();
                     Iterator<Integer> iter = edges.edgeIterator();
                     while(iter.hasNext()) {
                         final int target = iter.next();
                         contib.add(new Tuple2(target,
                                 currentRank/ (double)edges.getNEdges()));
                     }
                     return contib;
                 });

         return result.reduceByKey((Double d1, Double d2) -> d1 + d2)
